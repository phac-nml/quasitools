...
import decimal
import pysam
from quasitools.utilities import sam_alignment_to_padded_alignment, pairwise_alignment_to_differences
from quasitools.mapped_read import MappedRead, MappedReadCollection

def parse_mapped_reads_from_bam(reference, overlap_cutoff, identity_cutoff, bam):
    ...
    for alignment in sam.fetch(reference=reference.name):
        ...
        if overlap >= overlap_cutoff:
            ...
            if identity >= identity_cutoff:
                ...
                mrc.mapped_reads["{0}_{1}".format(alignment.query_name, '1' if direct == '+' else '2')] = mapped_read
